...
PLATFORM_LOADERS := $(foreach p,$(INSTALLER_PLATFORMS),$(shell $(ONL_PKG_INSTALL) platform-$(p):amd64 --find-file onl.$(p).loader))
...
$(INSTALLER_NAME).cpio: $(PLATFORM_DIRS) $(INSTALLER_SWI)
	...
	$(ONL_V_at)cp $(INSTALLER_SWI) onl-amd64.swi
	$(ONL_V_GEN)set -o pipefail ;\
	if $(ONL_V_P); then v="-v"; else v="--quiet"; fi ;\
	find *.loader lib onl-amd64.swi \
	| cpio $$v -H newc -o > $@
	$(ONL_V_at)rm -f onl-amd64.swi
